...
export class PriorityField extends Component {
    static template = "stock_priority.PriorityField";
    static props = {
        ...standardFieldProps,
    ...
    setup() {
        this.fa_list = ['star-o', 'star', 'flag', 'play fa-rotate-270', 'square', 'circle'];
        this.state = useState({
            index: -1,
        });
    ...
    get fa() {
        return this.fa_list[this.index];
    }
    ...
    onFaClicked() {
        ...
        if (_index >= this.options.length - 1){
            _index = -1;
        }
        this.updateRecord(String(_index + 1));
    }
    getTip() {
        return this.options[this.index][1];
    }
...
registry.category("fields").add("priority_new", priorityField);
